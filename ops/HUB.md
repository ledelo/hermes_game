...
- **3단 모델 라우팅 (2026-08-14 사용자 지시 확정)**:
  ...
  - 구조 제약(정직 기록): 트리거 self-bind라 틱 웨이크 자체는 Fable 고정 — 프로브 1콜짜리 틱은 위임 시 총비용 증가라 Fable이 직접 하되 최소 동작만. 경량 틱의 실질 절감은 웨이크 빈도 감축으로 달성 (대기 중 2시간 간격, 연동 감지 시 LAUNCH 단계에서 update_trigger로 매시간 복원 — 복원 명령: cron '0 * * * *').
