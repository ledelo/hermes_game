...
- **틱 라우팅 세분화 (2026-08-14 사용자 지적 반영)**: ①경량 틱(프로브 1콜) = Fable 직접 — Opus 스폰 오버헤드(2만+토큰)가 틱 비용(2천)보다 커서 위임이 손해임을 실측. ②중량 틱(리서치+배치실사+파싱) = **Opus 서브에이전트 위임** (서브에이전트의 세션 MCP 호출 가능 검증됨). ③스토어 쓰기(등록/주문수락/고객응대 발송) = 자금·법적 책임이 걸린 판단이라 Fable 유지, 준비물 생성(상세 HTML 등)은 Opus. 제약: 이 세션 자체는 Fable 고정(트리거 self-bind), 새 세션 발화는 커넥터 미탑재로 불가.
