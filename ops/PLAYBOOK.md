...
- **가격 교정은 반드시 큐 테이블 본문 셀을 직접 수정한다 (배치 요약 각주에만 적지 말 것)**. 2026-08-15 저녁 발견(2건) → 2026-08-16 전수 감사로 확산 확인: 1~9차 배치의 교정 28건 중 **26건이 테이블 셀에 전혀 반영 안 되어 있었음** (그중 6건은 옛 가격 그대로 두면 역마진 -29.2%~-2.5%). 전부 수정 완료(28/28 일치, 스크립트 검증). LAUNCH는 테이블 셀을 읽으므로 각주만으로는 절대 보호되지 않는다 — **매 배치 실사 직후 그 자리에서 테이블 셀까지 수정을 완료해야 하며, "요약만 쓰고 테이블은 나중에" 패턴을 금지한다.**
